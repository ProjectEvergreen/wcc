  ...
  for(const node of tree.childNodes) {
    if(node.tagName && node.tagName.indexOf('-') > 0) {
      ...
      if(node.childNodes.length === 0) {
        const { tagName } = node;
        const { moduleURL, instanceName } = deps[tagName]; // .moduleURL;
        const elementInstance = await initializeCustomElement(moduleURL, tagName);
        const shadowRootHtml = elementInstance.getInnerHTML({ includeShadowRoots: true }).trim();
...
async function initializeCustomElement(elementURL, tagName) {
...
async function renderToString(elementURL) {
  ...
  const componentHtml = elementInstance.getInnerHTML({ includeShadowRoots: true }).trim();
  const componentTree = parseFragment(componentHtml);
  const finalTree = await renderComponentRoots(componentTree);

  elementInstance.shadowRoot.innerHTML = serialize(finalTree);

  return elementInstance.getInnerHTML({ includeShadowRoots: false });
}
