...
async function renderToString(elementURL) {
  const moduleContents = await fs.readFile(elementURL, 'utf-8');
  ...
  for(const dep in deps) {
    const x = new (globalThis.customElements.get(dep))();
  ...
  return elementInstance.mode === 'closed'
    ? elementInstance.shadowRoot.innerHTML
    : `<template shadowroot="open">
        ${elementInstance.shadowRoot.innerHTML}
      </template>`;
}
