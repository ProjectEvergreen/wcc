...
describe('Run WCC For ', function () {
  ...
  before(async function () {
    ...
    dom = new JSDOM(html);
    pageContentsDom = new JSDOM(
      dom.window.document.querySelectorAll('wcc-home template[shadowrootmode="open"]')[0].innerHTML,
    );
  });
  ...
  describe(LABEL, function () {
    it('should have one top level <template> with an open shadowroot', function () {
      expect(
        dom.window.document.querySelectorAll('template[shadowrootmode="open"]').length,
      ).to.equal(1);
    ...
    describe('custom footer element', function () {
      ...
      before(function () {
        footerContentsDom = new JSDOM(
          pageContentsDom.window.document.querySelectorAll(
    ...
    describe('static page content', function () {
      it('should have the expected static content for the page', function () {
        expect(pageContentsDom.window.document.querySelector('h1').textContent).to.equal(
          'Home Page',
        );
      });
    });
    ...
    describe('custom header element with nested navigation element', function () {
      ...
      before(function () {
        headerContentsDom = new JSDOM(
          pageContentsDom.window.document.querySelectorAll(
            'wcc-header template[shadowrootmode="open"]',
          )[0].innerHTML,
        );
      });
      ...
      describe('nested navigation element', function () {
        ...
        before(function () {
          navigationContentsDom = new JSDOM(
            headerContentsDom.window.document.querySelectorAll(
              'wcc-navigation template[shadowrootmode="open"]',
            )[0].innerHTML,
          );
        });
