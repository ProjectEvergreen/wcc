/*
 * Use Case
 * Run wcc against a single custom element that uses a custom extension for a CSS file.
 *
 * User Result
...
describe('Run WCC For ', function () {
  ...
  before(async function () {
    const { html } = await renderFromHTML('<wcc-footer></wcc-footer>', [
  ...
  describe(LABEL, function () {
    ...
    it('should have one top level <wcc-footer> element with a <template> with an open shadowroot', function () {
      expect(
        dom.window.document.querySelectorAll('wcc-footer template[shadowrootmode="open"]').length,
      ).to.equal(1);
