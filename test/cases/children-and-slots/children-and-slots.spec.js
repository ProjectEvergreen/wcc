...
describe('Run WCC For ', function () {
  ...
  describe(LABEL, function () {
    it('should have one two top level <wcc-paragraph> custom elements with a <template> with an open shadowroot', function () {
      expect(
        dom.window.document.querySelectorAll('wcc-paragraph template[shadowrootmode="open"]')
          .length,
      ).to.equal(2);
    ...
    describe('<my-paragraph> with default <slot> content', function () {
      ...
      before(function () {
        paragraphContentsDom = new JSDOM(
          dom.window.document.querySelectorAll(
            'wcc-paragraph.default template[shadowrootmode="open"]',
          )[0].innerHTML,
        );
      });
      ...
      it('should have one <my-paragraph> tag with the default content', function () {
        expect(paragraphContentsDom.window.document.querySelector('p').textContent).to.equal(
          'My default text',
        );
      });
    });
    ...
    describe('<my-paragraph> with custom <slot> content', function () {
      ...
      before(function () {
        paragraphContentsDom = new JSDOM(
          dom.window.document.querySelectorAll(
            'wcc-paragraph.custom template[shadowrootmode="open"]',
          )[0].innerHTML,
        );
        paragraphContentsLightDom = new JSDOM(
          dom.window.document.querySelectorAll('wcc-paragraph.custom')[0].innerHTML,
        );
      });
      ...
      it('should have one <span> tag with the custom content in the light DOM', function () {
        expect(
          paragraphContentsLightDom.window.document.querySelector('span').textContent,
        ).to.equal("Let's have some different text!");
      });
    });
  });
});
